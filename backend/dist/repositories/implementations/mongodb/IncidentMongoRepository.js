...
exports.IncidentMongoRepository = void 0;
const mongodb_1 = require("mongodb");
const logger_1 = __importDefault(require("../../../utils/logger"));
class IncidentMongoRepository {
    constructor(client) {
        this.client = client;
    ...
    async findById(id) {
        try {
            const doc = await this.incidentsCollection.findOne({ _id: new mongodb_1.ObjectId(id) });
            if (!doc) {
                return null;
            }
            // Get the health check data
            const healthCheck = await this.healthChecksCollection.findOne({ _id: new mongodb_1.ObjectId(doc.healthCheckId) }, { projection: { name: 1, type: 1 } });
    ...
    async create(incident) {
        try {
            ...
            let healthCheckId = incident.healthCheckId;
            try {
                healthCheckId = new mongodb_1.ObjectId(incident.healthCheckId);
            }
            catch (error) {
    ...
    async update(id, data) {
        try {
            ...
            await this.incidentsCollection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: updateData });
    ...
    // Get incident events
    async getEvents(incidentId) {
        try {
            const events = await this.eventsCollection
                .find({ incidentId: new mongodb_1.ObjectId(incidentId) })
                .sort({ createdAt: -1 })
                .toArray();
    ...
    async addEvent(event) {
        try {
            // Convert string ID to ObjectId for MongoDB
            let incidentId = event.incidentId;
            try {
                incidentId = new mongodb_1.ObjectId(event.incidentId);
            }
            catch (error) {
...
exports.IncidentMongoRepository = IncidentMongoRepository;
//# sourceMappingURL=IncidentMongoRepository.js.map